...
import {
  buildRegionInsightUserPrompt,
  REGION_INSIGHT_SYSTEM_PROMPT,
  type RegionInsightContext,
} from "@/lib/region-excel/build-region-insight-context";

export type GenerateRegionInsightsResult = {
  items: string[];
...
export async function generateRegionInsightsWithHf(
  context: RegionInsightContext,
  fallbackItems: string[],
): Promise<GenerateRegionInsightsResult> {
  ...
  try {
    ...
    const response = await fetch(config.apiUrl, {
      ...
      body: JSON.stringify({
        model: config.model,
        messages: [
          { role: "system", content: REGION_INSIGHT_SYSTEM_PROMPT },
          { role: "user", content: buildRegionInsightUserPrompt(context) },
        ],
